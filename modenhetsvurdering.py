...
Bane NOR - Konsern okonomi og digital transformasjon
Version: Clean (uten ikoner)
"""
...
"""
    ...
    html += "<h2>7. Intervjuoversikt</h2><table><tr><th>Dato</th><th>Intervjuobjekt</th><th>Gevinst</th><th>Fase</th><th>Snitt</th></tr>"
    for interview in initiative.get('interviews', {}).values():
        ...
        html += f"<tr><td>{info.get('date', '')}</td><td>{info.get('interviewee', '')}</td><td>{info.get('benefit_name', 'Generelt')}</td><td>{info.get('phase', '')}</td><td>{avg:.2f if avg > 0 else '-'}</td></tr>"
    html += "</table>"
...
def generate_word_report(initiative, stats):
    ...
    try:
        ...
        doc.add_paragraph('Bane NOR - Konsern okonomi og digital transformasjon').alignment = WD_ALIGN_PARAGRAPH.CENTER
        
        doc.add_heading('1. Sammendrag', level=1)
        table = doc.add_table(rows=5, cols=2)
        table.style = 'Table Grid'
        cells = [('Endringsinitiativ', initiative['name']), ('Beskrivelse', initiative.get('description', '-')),
                 ('Rapportdato', datetime.now().strftime('%d.%m.%Y')), ('Antall intervjuer', str(stats['total_interviews'])),
        ...
        if stats['phases']:
            ...
            table.style = 'Table Grid'
            for i, h in enumerate(['Fase', 'Gjennomsnitt', 'Min', 'Maks']):
        ...
        if stats['high_maturity']:
            doc.add_heading('3. Styrkeomrader', level=1)
            for item in stats['high_maturity'][:15]:
                doc.add_paragraph(f"[{item['phase']}] {item['title']}: {item['score']:.2f}")
        
        if stats['low_maturity']:
            doc.add_heading('4. Forbedringsomrader', level=1)
            for item in stats['low_maturity'][:15]:
                doc.add_paragraph(f"[{item['phase']}] {item['title']}: {item['score']:.2f}")
...
def generate_pdf_report(initiative, stats):
    ...
    try:
        ...
        styles.add(ParagraphStyle(name='Title2', fontSize=18, alignment=TA_CENTER, spaceAfter=10))
        styles.add(ParagraphStyle(name='Subtitle2', fontSize=12, alignment=TA_CENTER, spaceAfter=20, textColor=colors.HexColor('#0053A6')))
        
        story = []
        story.append(Paragraph('Modenhetsvurdering - Gevinstrealisering', styles['Title2']))
        story.append(Paragraph('Bane NOR', styles['Subtitle2']))
        story.append(Spacer(1, 20))
        
        data = [['Endringsinitiativ', initiative['name']], ['Rapportdato', datetime.now().strftime('%d.%m.%Y')],
                ['Antall intervjuer', str(stats['total_interviews'])], ['Samlet modenhet', f"{stats['overall_avg']:.2f}"]]
        t = Table(data, colWidths=[5*cm, 10*cm])
        t.setStyle(TableStyle([('GRID', (0, 0), (-1, -1), 0.5, colors.grey), ('PADDING', (0, 0), (-1, -1), 8)]))
        story.append(t)
        
        if stats['phases']:
            story.append(Spacer(1, 20))
            story.append(Paragraph('Modenhet per fase', styles['Heading2']))
            phase_data = [['Fase', 'Snitt']] + [[p, f"{d['avg']:.2f}"] for p, d in stats['phases'].items()]
            t = Table(phase_data, colWidths=[6*cm, 3*cm])
            t.setStyle(TableStyle([('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#0053A6')), ('TEXTCOLOR', (0, 0), (-1, 0), colors.white),
                                   ('GRID', (0, 0), (-1, -1), 0.5, colors.grey), ('PADDING', (0, 0), (-1, -1), 6)]))
            story.append(t)
        
        story.append(Spacer(1, 30))
        story.append(Paragraph(f"Generert {datetime.now().strftime('%d.%m.%Y %H:%M')}", styles['Subtitle2']))
